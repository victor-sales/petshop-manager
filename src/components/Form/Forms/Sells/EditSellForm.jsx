...
export default function AddSellForm ({sell, setSell}) {

    const { token } = useAuthContext()
    const { handleUpdateSell } = useSellsContext()
    ...
    async function updateSell () {
        ...
        if (areValid) {
            await handleUpdateSell(token, sell)
        } else {
            return false
        } 
    }
    ...
    useEffect(() => {
        ...
        if (button) button.addEventListener("click", updateSell)

        return () => {
            const button = document.getElementById("confirm-button")
            if (button) button.removeEventListener("click", updateSell)
        }
        //eslint-disable-next-line
    }, [token, sell])
